...
const metaOf = (response) => response?.data?.meta || response?.meta || {};
    ...
    <Modal open={open} onClose={onClose}>
      ...
      >
        ...
        <SoftBox display="flex" gap={2} mt={4}>
          <SoftButton variant="outlined" color="secondary" onClick={onClose} fullWidth>
            Hủy
          </SoftButton>
          <SoftButton
            variant="gradient"
...
function HangHoa() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [firstProductResponse, catRes] = await Promise.all([
        ProductService.getAll({ page: 1, limit: 100 }),
        CategoryService.getAll(),
      ]);
      const totalPages = metaOf(firstProductResponse).totalPages || 1;
      const remainingResponses =
        totalPages > 1
          ...
          : [];
      const allProducts = listOf(firstProductResponse).concat(...remainingResponses.map(listOf));
  ...
  const handleExport = async () => {
    try {
      ...
      toast.error(error.response?.data?.message || "Không thể xuất danh sách sản phẩm");
    }
  };
    ...
    try {
      ...
      toast.success(
        `Import ${result.totalRows || 0} dòng: thêm ${result.created || 0}, cập nhật ${
          result.updated || 0
        }, tạo ${result.categoriesCreated || 0} danh mục, lỗi ${result.failed || 0}`
      );
      if (result.errors?.length)
        exportExcel(
          ...
          `loi-import-san-pham-${Date.now()}.xlsx`,
          "Loi import"
        );
      load();
    } catch (error) {
