...
function AdvanceTab() {
  ...
  const load = () =>
    GoodsAdvanceService.list({ page: 1, limit: 100 })
  ...
  const save = async (status) => {
    if (!form.employee || !form.truck || !items.length)
      return toast.error("Chọn nhân viên, xe và hàng tạm ứng");
    try {
      setSaving(true);
      await GoodsAdvanceService.create({
        ...
        warehouseIssuerName: "Thủ kho",
        advanceRecipientName: form.employee.fullName || form.employee.username,
        status,
      });
      toast.success(status === "CONFIRMED" ? "Đã xác nhận tạm ứng hàng" : "Đã lưu bản nháp");
      setItems([]);
      setForm({ date: today(), employee: null, truck: null, note: "", issues: "" });
      load();
    } catch (error) {
      toast.error(error.response?.data?.message || "Không thể lưu phiếu tạm ứng");
    } finally {
      setSaving(false);
    }
  };
  const changeStatus = async (doc, status) => {
    try {
      await GoodsAdvanceService.changeStatus(
        idOf(doc),
        status,
        status === "CANCELLED" ? "Admin hủy phiếu" : undefined
      );
      await load();
    } catch (error) {
    ...
    <SoftBox>
      <SoftBox p={{ xs: 1.5, md: 2.5 }} borderRadius={3} bgcolor="#f3f8ff" mb={2}>
        ...
        <Grid container spacing={1.5} mt={0.25}>
          ...
          <Grid item xs={12} md={4}>
            <Autocomplete
              ...
              renderInput={(params) => <TextField {...params} placeholder="Nhân viên nhận hàng" />}
            />
          </Grid>
          <Grid item xs={12} md={4}>
            <Autocomplete
              ...
              renderInput={(params) => <TextField {...params} placeholder="Xe nhận hàng" />}
            />
          </Grid>
          <Grid item xs={12}>
            <Autocomplete
              value={null}
              options={products.filter((p) => !selectedIds.has(idOf(p)))}
              ...
              renderInput={(params) => (
                <TextField {...params} placeholder="Tìm và thêm hàng hóa..." />
              )}
            />
          </Grid>
        </Grid>
        {items.map((item, index) => (
          <SoftBox
            key={idOf(item.product)}
            mt={1}
            p={1.25}
            bgcolor="#fff"
            borderRadius={2}
            sx={{ border: "1px solid #d7e5f5" }}
          >
            ...
            <SoftBox display="flex" alignItems="center" gap={1} mt={1}>
              <IconButton
                onClick={() => patchItem(index, { quantity: Math.max(1, item.quantity - 1) })}
              >
                <Icon>remove</Icon>
              </IconButton>
              <SoftInput
                ...
                value={item.quantity}
                onChange={(e) =>
                  patchItem(index, { quantity: Math.max(1, Number(e.target.value)) })
                }
                sx={{ maxWidth: 100, "& input": { textAlign: "center" } }}
              />
              <IconButton onClick={() => patchItem(index, { quantity: item.quantity + 1 })}>
                <Icon>add</Icon>
              </IconButton>
              <SoftInput
                value={item.note}
        ...
        <Grid container spacing={1.5} mt={0.5}>
          ...
          <Grid item xs={12} md={6}>
            <SoftInput
              ...
              placeholder="Các vấn đề cần chú ý"
            />
          </Grid>
        </Grid>
      ...
      <Grid container spacing={1.5}>
        {data.map((doc) => {
          const badge = statusStyle[doc.status] || [doc.status, "#eee", "#555"];
          return (
            <Grid item xs={12} md={6} lg={4} key={idOf(doc)}>
              <SoftBox p={1.5} borderRadius={2} sx={{ border: "1px solid #e1e6ee" }}>
                ...
                <SoftBox display="flex" gap={0.5} mt={1}>
                  <SoftButton size="small" color="info" onClick={() => exportDoc(doc)}>
                    Xuất file
                  </SoftButton>
                  {doc.status === "DRAFT" && (
                    <>
                      <SoftButton
                        size="small"
                        color="success"
                        onClick={() => changeStatus(doc, "CONFIRMED")}
                      >
                        Xác nhận
                      </SoftButton>
                      <SoftButton
                        size="small"
                        color="error"
                        onClick={() => changeStatus(doc, "CANCELLED")}
                      >
                        Hủy
                      </SoftButton>
                    </>
...
function DailyReportTab() {
  ...
  const loadList = () =>
    DailyReportService.list({ page: 1, limit: 100 }).then((r) => setSaved(rows(r)));
  const loadPreview = () =>
    DailyReportService.preview(date)
      .then((r) => setPreview(unwrap(r)))
      .catch((e) => toast.error(e.response?.data?.message || "Không thể tổng hợp báo cáo"));
  useEffect(() => {
    DailyReportService.preview(today())
      .then((response) => setPreview(unwrap(response)))
      .catch((error) => toast.error(error.response?.data?.message || "Không thể tổng hợp báo cáo"));
    DailyReportService.list({ page: 1, limit: 100 }).then((response) => setSaved(rows(response)));
  }, []);
  ...
  const create = async () => {
    try {
      ...
      toast.error(e.response?.data?.message || "Không thể chốt báo cáo");
    }
  };
  const exportDoc = async (doc) => {
    const r = await DailyReportService.export(idOf(doc));
    downloadBlob(r.data, `${doc.code}.xlsx`);
  };
  return (
    <SoftBox>
      <SoftBox display="flex" gap={1} flexWrap="wrap" mb={2}>
        <SoftInput
          ...
          sx={{ maxWidth: 220 }}
        />
        <SoftButton color="info" onClick={loadPreview}>
          <Icon>refresh</Icon>&nbsp;Tổng hợp lại
        </SoftButton>
        <SoftButton color="success" onClick={create}>
          <Icon>lock</Icon>&nbsp;Chốt báo cáo
        </SoftButton>
      </SoftBox>
      {preview && (
        <>
          <Grid container spacing={1.25}>
            {[
              ["Chứng từ", summary.documentCount],
              ["Doanh thu bán", money(summary.salesRevenue)],
              ["Tiền mặt", money(summary.cash)],
              ["Chuyển khoản", money(summary.bankTransfer)],
              ["Doanh thu ròng", money(summary.netRevenue)],
            ].map(([label, value]) => (
              <Grid item xs={6} md key={label}>
                <SoftBox p={1.5} borderRadius={2} bgcolor="#f7f9fc">
              ...
              {(preview.documents || []).map((doc) => (
                ...
                >
                  <SoftBox display="flex" alignItems="center" gap={1}>
                    ...
                    <SoftBox>
                      <SoftTypography variant="button" fontWeight="bold">
                        {doc.code} · {doc.customerName || "Khách lẻ"}
                      </SoftTypography>
                      <SoftTypography variant="caption" display="block" color="text">
                        {doc.employeeName} · {doc.type}
                      </SoftTypography>
                    </SoftBox>
                  </SoftBox>
                  <SoftTypography variant="button" fontWeight="bold">
              ...
              {(preview.products || []).map((p) => (
                ...
                >
                  <SoftBox display="flex" alignItems="center" gap={1}>
                    ...
                    <SoftTypography variant="button">{p.productName}</SoftTypography>
                  </SoftBox>
                  <SoftTypography variant="button" fontWeight="bold">
                    {p.quantity} {p.unit}
                  </SoftTypography>
          ...
          <Grid container spacing={1.5} mt={1}>
            ...
            <Grid item xs={12} md={6}>
              <SoftInput
                ...
                placeholder="Vấn đề cần giải quyết ngay"
              />
            </Grid>
          </Grid>
        </>
      ...
      <SoftBox py={3}>
        <SoftBox bgcolor="#fff" borderRadius={3} p={{ xs: 1.5, md: 3 }}>
          ...
          <Tabs value={tab} onChange={(_, value) => setTab(value)} sx={{ mb: 2 }}>
            <Tab label="Phiếu tạm ứng hàng" />
            <Tab label="Báo cáo tổng hợp ngày" />
          </Tabs>
          {tab === 0 ? <AdvanceTab /> : <DailyReportTab />}
        </SoftBox>
      </SoftBox>
    </DashboardLayout>
  );
