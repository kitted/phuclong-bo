...
function NhapKho() {
  ...
  const [form, setForm] = useState({ code: "", supplierId: "", note: "", date: new Date().toISOString().split("T")[0] });
  ...
  const load = async () => {
    ...
    try {
      ...
      const impData = impRes?.data || impRes || [];
  ...
  const totalAmount = items.reduce((sum, i) => sum + (Number(i.qty) || 0) * (Number(i.price) || 0), 0);

  const handleSubmit = async () => {
    // SỬA Ở ĐÂY: Thêm điều kiện kiểm tra Mã phiếu
    if (!form.code.trim()) { toast.error("Vui lòng nhập mã phiếu"); return; }
    ...
    try {
      ...
      toast.error(e.response?.data?.message || "Có lỗi xảy ra khi tạo phiếu nhập (Kiểm tra lại mã phiếu có thể bị trùng)");
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <SoftBox p={3}>
            ...
            <SoftBox sx={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {loading && <tr><td colSpan={6} style={{ textAlign: "center", padding: 32, color: "#9E9E9E" }}>Đang tải dữ liệu...</td></tr>}
                  {!loading && imports.length === 0 && <tr><td colSpan={6} style={{ textAlign: "center", padding: 32, color: "#9E9E9E" }}>Chưa có phiếu nhập nào</td></tr>}
                  ...
                  {!loading && imports.map((imp, idx) => {
                    ...
                    return (
                      <tr key={impId} style={{ borderBottom: "1px solid #F0F0F0", background: idx % 2 === 0 ? "#fff" : "#FAFAFA" }}>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
        <SoftBox sx={{
          ...
          <Grid container spacing={2} mb={2}>
            ...
            <Grid item xs={6} md={4}>
              <SoftTypography variant="caption" fontWeight="medium">Mã phiếu *</SoftTypography>
              <SoftInput value={form.code} onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))} placeholder="VD: PN001" fullWidth autoFocus />
            </Grid>
            ...
            <Grid item xs={12}>
              ...
              <SoftInput value={form.note} onChange={(e) => setForm((f) => ({ ...f, note: e.target.value }))} placeholder="Nhập lý do hoặc mã chứng từ gốc..." fullWidth />
            </Grid>
          </Grid>

          {/* Dòng Hàng Hóa */}
          <SoftTypography variant="button" fontWeight="bold" mb={1} display="block" mt={3}>Danh sách hàng hóa</SoftTypography>
          {items.map((item, idx) => {
            return (
              <SoftBox key={idx} display="flex" gap={1} alignItems="center" mb={1.5}>
                <FormControl size="small" sx={{ flex: 2 }}>
                  <Select value={item.productId || ""} onChange={(e) => handleItemChange(idx, "productId", e.target.value)} displayEmpty sx={{ height: 40 }}>
                    ...
                    {products.map((p) => <MenuItem key={p.id || p._id} value={p.id || p._id}>{p.name} ({p.unit})</MenuItem>)}
                  </Select>
                </FormControl>
                ...
                <SoftBox sx={{ flex: 1 }}>
                  <SoftInput type="number" value={item.price} onChange={(e) => handleItemChange(idx, "price", e.target.value)} placeholder="Giá nhập" />
                </SoftBox>
                ...
                <IconButton size="small" onClick={() => handleRemoveItem(idx)} disabled={items.length === 1}>
                  <Icon sx={{ color: items.length === 1 ? "#E0E0E0" : "#EF4444" }}>remove_circle</Icon>
                </IconButton>
              </SoftBox>
            );
          })}
          ...
          <SoftBox display="flex" justifyContent="flex-end" mb={3} mt={2} pt={2} sx={{ borderTop: "1px dashed #E0E0E0" }}>
            <SoftTypography variant="h6" fontWeight="bold">Tổng cộng: {fmtCurrency(totalAmount)}</SoftTypography>
          </SoftBox>

          <SoftBox display="flex" gap={2}>
            <SoftButton variant="outlined" color="secondary" onClick={() => setModalOpen(false)} fullWidth>Hủy bỏ</SoftButton>
        ...
        <Modal open={!!detailModal} onClose={() => setDetailModal(null)}>
          <SoftBox sx={{
            ...
            <SoftBox display="flex" justifyContent="space-between" alignItems="center" mb={1}>
              <SoftTypography variant="h5" fontWeight="bold">Phiếu: {detailModal.code}</SoftTypography>
            ...
            <SoftBox mb={3} p={2} sx={{ background: "#F8F9FA", borderRadius: 2 }}>
              ...
              <SoftTypography variant="body2" color="text">
                <strong>Nhà cung cấp:</strong> {
                  detailModal.supplierId?.name || suppliers.find(s => (s.id || s._id) === extractId(detailModal.supplierId))?.name || "—"
                }
              </SoftTypography>
              {detailModal.note && (
                <SoftTypography variant="body2" color="text">
            ...
            <SoftTypography variant="button" fontWeight="bold" display="block" mb={1}>Chi tiết hàng hóa</SoftTypography>
            <SoftBox sx={{ maxHeight: 250, overflowY: "auto", pr: 1 }}>
              {(detailModal.items || []).map((item, i) => {
                ...
                return (
                  <SoftBox key={i} display="flex" justifyContent="space-between" alignItems="center" py={1.5} sx={{ borderBottom: "1px solid #f0f0f0" }}>
                    <SoftBox>
                      <SoftTypography variant="button" display="block">{pName}</SoftTypography>
            ...
            <SoftBox mt={3}>
              <SoftButton variant="outlined" color="secondary" fullWidth onClick={() => setDetailModal(null)}>
                Đóng
              </SoftButton>
            </SoftBox>
          </SoftBox>
