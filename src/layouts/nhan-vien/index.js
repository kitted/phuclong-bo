...
function EmployeeForm({ open, employeeId, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    if (!employeeId && form.password.length < 6)
      return toast.error("Mật khẩu phải có ít nhất 6 ký tự");
    if (employeeId && form.password && form.password.length < 6)
    ...
    <Modal open={open} onClose={onClose}>
      ...
      >
        <SoftTypography variant="h5" fontWeight="bold">
          {employeeId ? "Cập nhật nhân viên" : "Thêm nhân viên"}
        </SoftTypography>
        <SoftTypography variant="caption" color="text">
          Tài khoản được cấp quyền Staff
        </SoftTypography>
        {loadingDetail ? (
          ...
          <Grid container spacing={2} mt={1}>
            <FormGridField label="Mã nhân viên">
              <SoftInput
            ...
            <FormGridField label="Tên đăng nhập *">
              <SoftInput
                value={form.username || ""}
            ...
            <FormGridField label={employeeId ? "Mật khẩu mới" : "Mật khẩu *"}>
              <SoftInput
                type="password"
                value={form.password || ""}
                onChange={(e) => set("password", e.target.value)}
                placeholder={employeeId ? "Để trống nếu không đổi" : "Tối thiểu 6 ký tự"}
                fullWidth
              />
            </FormGridField>
            <FormGridField label="Số điện thoại">
              <SoftInput
                value={form.phone || ""}
            ...
            <FormGridField label="Ghi chú" md={12}>
              <SoftInput
                value={form.note || ""}
...
export default function NhanVien() {
  ...
  const [assignKpiEmployee, setAssignKpiEmployee] = useState(null);
  ...
  const changeStatus = async (employee) => {
    ...
    try {
      await EmployeeService.changeStatus(getId(employee), next);
  ...
  const remove = async (employee) => {
    if (!window.confirm(`Xóa nhân viên ${employee.fullName || employee.username}?`)) return;
    try {
  ...
  return (
    <DashboardLayout>
          ...
          <SoftBox p={3}>
            ...
            >
              <SoftBox>
                ...
                <SoftTypography variant="caption" color="text">
                  Tài khoản và quyền truy cập dành cho Staff
                </SoftTypography>
              </SoftBox>
              <SoftButton
                variant="gradient"
            ...
            <SoftBox display="flex" gap={2} mb={3} flexWrap="wrap">
              <SoftBox sx={{ flex: 1, minWidth: 240 }}>
                <SoftInput
                  ...
                  placeholder="Tìm mã, username, họ tên, điện thoại..."
      ...
      />
      <AssignKpiModal
        open={Boolean(assignKpiEmployee)}
        employee={assignKpiEmployee}
        onClose={() => setAssignKpiEmployee(null)}
        onSaved={() => {
          const employee = assignKpiEmployee;
          setAssignKpiEmployee(null);
          setProgressEmployee(employee);
        }}
      />
      <KpiProgressModal
        open={Boolean(progressEmployee)}
